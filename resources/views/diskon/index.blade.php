...
<body>
    ...
    <div class="header-area" id="headerArea">
        <div class="container">
            <div class="header-content header-style-three position-relative d-flex align-items-center justify-content-between">
                ...
                <div class="logo-wrapper"><a href="#" class="title-page">Daftar Diskon</a></div>
                <div class="user-profile-wrapper">
                    <a href="{{ route('tambahDiskon') }}" class="text-black">
    ...
    <div class="page-content-wrapper">
        ...
        <section id="list-transaksi-header">
            <div class="container">
                <div class="pb-3 pt-3">
                    <input type="text" class="form-control" placeholder="Cari diskon" id="search">
                </div>
            </div>
        </section>
        ...
        <div class="mt-3"></div>
        <div class="container">
            <table class="w-100" id="listDiskon">
                <thead>
                <tr>
                    <th></th>
                </tr>
                ...
                <tbody>
                @foreach($diskon as $par)
                    <tr>
                        <td>
                            <div class="card p-2 mb-3">
                                <p class="text-nama">{{ $par->nama }}</p>
                                <p class="text-harga">{{ $par->type }}</p>
                                <p class="text-harga">
                                    @if($par->type == "nominal")
                                        Rp.{{ number_format($par->nominal) }}
                                    @else
                                        {{ number_format($par->nominal) }} %
                                    @endif
                                </p>
                                <div class="row mt-3">
                                    <div class="col-10">
                                        <a href="/edit-diskon/{{ base64_encode($par->id) }}" class="btn btn-putih">Ubah</a>
                                    </div>
                                    <div class="col-2">
    ...
    <script>
        let table = $("#listDiskon").DataTable({
            paging: false,
            order: false
        });
