            ...
            <div class="col-12">
                <div class="card p-2">
                    <form action="{{ route('perpanjangLisensiProses') }}" method="POST">
                        ...
                        <div class="form-group mb-3">
                            <label for="nama" class="form-label">Expired Lisensi Outlet</label>
                            <input type="text" class="form-control" value="{{ formatTanggal($outlet->expired) }}" readonly>
                        </div>
                        <div class="form-group mb-3">
                            <label for="lisensi" class="form-label">Lisensi</label>
                            <select class="form-control" name="lisensi" id="lisensi" required>
                                <option value="">-- Pilih Lisensi --</option>
                                @foreach($lisensi as $l)
                                    <option value="{{ $l->id }}">{{ $l->nama }} | Rp{{ number_format($l->harga) }}</option>
                                @endforeach
                            </select>
                        </div>
